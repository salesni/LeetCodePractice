...
class Solution:
    def asteroidCollision(self, asteroids: List[int]) -> List[int]:
        ...
        for asteroid in asteroids:
            while result_stack and asteroid < 0  and result_stack[-1] > 0:
                last_item = result_stack.pop()

                if abs(asteroid) == abs(last_item):
                    break
                elif abs(asteroid) > abs(last_item):
                    continue
                else:
                    result_stack.append(last_item)
                    break
            else:
                 result_stack.append(asteroid)
